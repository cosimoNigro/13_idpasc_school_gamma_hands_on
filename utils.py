...
def plot_on_off_regions(observation, on_region, energies=None):
    ...
    pointing_radec = SkyCoord(
        ra=observation.obs_info["RA_PNT"] * u.deg,
        dec=observation.obs_info["DEC_PNT"] * u.deg,
        frame="icrs"
    )
    ...
    counts = Map.create(skydir=pointing_radec, binsz=0.06, width=4)
    ...
    # plot pointing
    pointing = PointSkyRegion(pointing_radec)
    ...
    off_regions, wcs = region_finder.run(
        region=on_region, center=pointing_radec
    )
